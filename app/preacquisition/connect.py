...
def get_network_ip_cidr(interface):
    ...
    try:
        ...
        loggers["network"].error(f"Error retrieving network IP and subnet for {interface}: {e}")
        return "192.168.0.0/24"  # Default fallback subnet
    ...
    while True:
        try:
            ...
            if enforcement_setting == "enable" and len(ip_addresses) > 2:
                loggers["network"].error("More than 2 devices detected. Network enforcement enabled. Aborting script...")
                exit_program()
