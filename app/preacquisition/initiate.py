    ...
    try:
        ...
        if not current_network_essid:
            loggers["network"].error(
                "Failed to retrieve the current network. Exiting setup.")
            return False, None
        loggers["network"].info(
            f"Currently connected to ESSID: {current_network_essid}\n")

        # Step 2: Perform a security check on the current network
        loggers["network"].info(
            "Performing security check on the current network...")
        if not iwlist_security_check(network_interface, current_network_essid):
            loggers["network"].error(
                "Security scan failed. Aborting connection setup.")
            return False, None
        ...
        while True:
            ...
            if ":" in router_input:
                ...
                if is_valid_ip(router_ip) and router_port.isdigit() and 1 <= int(router_port) <= 65535:
                    loggers["network"].info(
                        f"Checking firmware version for router at IP: {router_ip}, Port: {router_port}")
                    check_firmware_version(router_ip, router_port)
                    break
                else:
                    loggers["network"].error(
                        "Invalid IP or port entered. Please enter a valid IP and port (1-65535).")
            else:
                loggers["network"].error(
                    "Invalid format. Please enter in 'ip:port' format.")

        # Step 5: Pair and verify connection to the smartwatch
        loggers["network"].info(
            "Attempting to pair with the WearOS smartwatch...")
        success, watch_ip = pair_device()
        if not success:
            loggers["network"].error(
                "Failed to pair with the smartwatch. Ensure it's in pairing mode.")
            return False, None
        loggers["network"].info(
            f"Smartwatch paired successfully. Watch IP: {watch_ip}")

        # Step 6: Verify network devices and establish connection
        loggers["network"].info(
            "Verifying network devices and connecting to smartwatch...")

        # Step 7: Get the device's IP address
        loggers["network"].info("Fetching current device IP address...")
        current_device_ip = subprocess.getoutput(
        ...
        if verify_network_devices(current_device_ip, watch_ip, network_interface):
            if connect_to_device(watch_ip):
                loggers["network"].info(
                    "Successfully connected to the smartwatch.")
                return True, watch_ip
            else:
                loggers["network"].error(
                    "Failed to establish a connection with the smartwatch.")
        else:
        ...
        loggers["app"].warning(
            "Keyboard Interrupt: Setup process terminated by user.")
